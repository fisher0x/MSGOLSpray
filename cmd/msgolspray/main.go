...
func tryLogin(ctx context.Context, client *http.Client, username, password, url string, uaProvider func() string, xfwd bool) (string, bool, string) {
	...
	if uaProvider != nil {
		...
		req.Header.Set("User-Agent", "MSGOLSpray/1.0")
	}
	if xfwd {
...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "  -o, --output [dir]       Output file for successful results (JSON). If a directory or '.' is specified, saves as YYYY-MM-DD_HH-MM-SS_MSGOLSpray.json in that location.")
	...
	// Timestamp
	fmt.Printf("[%s] Starting MSGOLSpray\n", time.Now().Format("2006-01-02 15:04:05"))
	...
	if *saveLog != "" {
		...
		logName := fmt.Sprintf("%s_MSGOLSpray.log", now)
		logPath := *saveLog
	...
	default:
		uaProvider = func() string { return "MSGOLSpray/1.0" }
	}
	...
	if *output != "" {
		...
		now := time.Now().Format("2006-01-02_15-04-05")
		outName := fmt.Sprintf("%s_MSGOLSpray.json", now)
		outPath := *output
